    ...
    **Note**: Input batches must be tuples (input_tensor, target_tensor).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        torch.Tensor
            The output data from the forward pass.
        """
        x = self.backbone(x) #mandou o input pro backbone -> recebeu as representações
        if self.flatten:
            x = x.reshape(x.size(0), -1)
        if self.adapter is not None:
            x = self.adapter(x)
        x = self.fc(x)      #mandou as representações pra rede neural completamente conectada -> retorna seu retorno
        return x
    ...
    def _single_step(
        ...
        """
        x, y = batch    #x seria o input e y seria a máscara de segmetnação (a resposta)
        y_hat = self.forward(x) #mandou o x pelo modelo e recebeu a máscara de segmentação do modelo
        loss = self._loss_func(y_hat, y)    #calcula a "diferença entre as duas representações
