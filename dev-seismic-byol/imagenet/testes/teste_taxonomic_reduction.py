...
if __name__ == "__main__":
    ...
    for level in [3, 4, 5, 7, 6, 8, 9, 10, 11]:

        print(f"\n⚡ Disparando .to_coarse_classes(top_down={True}, level={level}) nos Readers...")
        
        try:
            num_classes_train = train_reader.to_coarse_classes(top_down=True, level=level, mat_path=meta_mat)
            num_classes_val = val_reader.to_coarse_classes(top_down=True, level=level, mat_path=meta_mat)
        ...
        wnid_to_coarse_val_check, _ = reduce_taxonomic_diversity(val_all_wnids, True, level, meta_mat)
