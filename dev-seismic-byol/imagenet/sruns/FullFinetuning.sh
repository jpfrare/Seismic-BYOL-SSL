        ...
        for d in "${finetune_dataset[@]}"; do
            for p in "${protocol[@]}"; do
                ...
                if [ "${p}" = 'full_finetuning' ]; then
                    ...
                    FLAGS="--reduction_mode ${red_mode} --version ${v} --backbone_freeze ${freeze} --pred_head ${head} --finetune_dataset ${d} --repetition ${r} --eval"
                    root=/petrobr/parceirosbr/home/joao.frare/workspace/spfm/Seismic-Byol/dev-seismic-byol/imagenet/jobs_out/imagenetFinetune/finetune_${d}/${red_mode}/repetition_${r}
                    mkdir -p "${root}"

        sbatch <<EOT
#!/bin/bash

#SBATCH --job-name=${red_mode}_d${d}_p${p}_v${v}_r${r}
#SBATCH --nodes=1
