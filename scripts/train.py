...
def load_pretrained_backbone(
    pretrained_backbone_checkpoint_filename, mode: str = "byol"
):
...
def build_downstream_datamodule(
    root_dir, batch_size, cap, data, seed
...
def build_lightning_trainer(
    save_name: str, supervised: bool, epocas, reps
) -> L.Trainer:
    ...
    return L.Trainer(
        ...
        max_epochs=epocas,
        logger=CSVLogger(
            "logs", name="Supervised" if supervised else "Pretrained", version=save_name
        ),
...
def train_func(
    ...
    if mode == "seg":
        ...
        pretrained_backbone_checkpoint_filename = (
            f"../saves/models/{aux}/{import_name}.ckpt"
        )
    else:
        pretrained_backbone_checkpoint_filename = (
            f"../saves/backbones/{repetition}/{import_name}.pth"
        )
    ...
    lightning_trainer = build_lightning_trainer(
        save_name, supervised, epocas, reps=repetition
    )
